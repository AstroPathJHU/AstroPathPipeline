...
In this section, we show plots that illustrate the results of the alignment procedure for the Vectra 3.0 microscope.
...
This section shows results for the Vectra Polaris microscope at BMS.

\subsection{Linear Transformation matrix}

From the sample named \texttt{TS18\_0541\_BMS\_MITRE}, we find the linear transformation matrix to be
\begin{align}
\matrixbold{A}=&\begin{pmatrix}
\num{1.00748\pm.00017} &
\num{-0.00261\pm.00018} \\
...
\end{align}
The Vectra Polaris scans fields of size $1876\times1404$ $\text{pixels}^2$, slightly larger than the Vectra 3.0.  The result is that when moving one field to the right, the microscope overshoots by an average of $14$ pixels.  When moving down, it overshoots by an average of $10$ pixels, and when moving back to the beginning of the row it overshoots by $5$ pixels on average.  If left uncorrected, this is a significantly larger bias than the one in the Vectra 3.0 microscope.
