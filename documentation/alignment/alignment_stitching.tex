...
\Cref{fig:alignmentresultsbig} shows the same distributions for a larger image taken with the same microscope.  It shows similar effects, though they are harder to see due to the larger number of overlaps.
...
\Cref{fig:2Dvectra} shows the two-dimensional distribution of the shifts for the \M11 dataset.  This plot highlights the sinusoidal dependence shown in the previous section and also reveals more features. in the 2D distribution.
...
Similar to \cref{sec:vectraperiodic}, we can look for periodic or other systematic behavior in the residuals from the Polaris microscope.  \Cref{fig:sinewavesJHUPolaris} shows plots for the \texttt{PZ1} dataset, equivalent to the ones in \cref{fig:sinewaves}.
...
Similar to \cref{sec:vectraperiodic}, we can look for periodic or other systematic behavior in the residuals from the Polaris microscope.  \Cref{fig:sinewavesBMS} shows plots for the \texttt{ML1603474\_BMS069\_5\_21} dataset, equivalent to the ones in \cref{fig:sinewaves}.

%These plots show less sinusoidal behavior than the ones for the Vectra 3.0 microscope, but they do show other effects.  \Cref{fig:sinewave2yyBMS} has a clear periodic mode, with a wavelength of approximately $5\times\text{the field height}$, though it does not fit to a simple sine wave.  Additionally, \cref{fig:sinewave2xyBMS} shows a behavior change at $y\approx\SI{10000}{pixels}$, which does not fit into any periodic pattern.
...
All four of the plots in the top row show two clusters of overlaps, indicating that there are systematic effects present in addition to the random movement.  The next sections will investigate those effects in greater detail.  The effects are smaller than the ones seen in the Vectra 3.0 microscope shown in \cref{fig:alignmentresults,fig:alignmentresultsbig} and are corrected by the stitching, as shown in the bottom row.
...
\end{align}
The Vectra Polaris scans fields of size $1876\times1404$ $\text{pixels}^2$, slightly larger than the Vectra 3.0.  The result is that when moving one field to the right, the microscope overshoots by an average of $0.7$ pixels, as expected from \cref{fig:alignmentresultAKY4}, and when moving down, it ends up $0.5$ pixels to the left of the desired location, as expected from \cref{fig:alignmentresultAKY2}.
...
\Cref{fig:2DAKY} shows the two-dimensional distribution of the shifts for the \texttt{ML1603474\_BMS069\_5\_21} dataset.  This dataset is quite stable and only shows a $1$-pixel settling effect at the beginning of the scan.
