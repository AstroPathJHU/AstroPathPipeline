...
\Cref{fig:2Dpolaris} shows the two-dimensional distribution of the shifts for the \texttt{PZ1} dataset.  This plot explains the features observed on the left side of \cref{fig:sinewavexyJHUPolaris}: at the beginning of the scan, the microscope took some time to gradually ``settle in'' to its final position.  The alignment procedure accounts for this settling.
...
\begin{table}[ht]
	...
	\multirowcell{2}[0pt][l]{RMS of shifts\\(pixels)}
	& $\delta x$ & $1.1$ & $0.8$ & $X.Y$ & $0.4$ \\\cline{2-6}
	& $\delta y$ & $2.0$ & $0.2$ & $X.Y$ & $0.3$ \\\hline
	\end{tabular}
