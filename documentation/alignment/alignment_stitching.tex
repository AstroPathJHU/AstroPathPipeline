...
Similar to \cref{sec:vectraperiodic}, we can look for periodic or other systematic behavior in the residuals from the Polaris microscope.  \Cref{fig:sinewavesJHUPolaris} shows plots for the \texttt{L1\_4} dataset, equivalent to the ones in \cref{fig:sinewaves}.

%These plots show less sinusoidal behavior than the ones for the Vectra 3.0 microscope, but they do show other effects.  \Cref{fig:sinewave2yyJHUPolaris} has a clear periodic mode, with a wavelength of approximately $5\times\text{the field height}$, though it does not fit to a simple sine wave.  Additionally, \cref{fig:sinewave2xyJHUPolaris} shows a behavior change at $y\approx\SI{10000}{pixels}$, which does not fit into any periodic pattern.
...
\end{align}
The Vectra Polaris scans fields of size $1876\times1404$ $\text{pixels}^2$, slightly larger than the Vectra 3.0.  The result is that when moving one field to the right, the microscope overshoots by an average of $0.8$ pixels.  When moving down, it overshoots by an average of $0.5$ pixels.
...
\begin{table}[ht]
	...
	\multirowcell{4}[0pt][l]{Sine amplitude \& \\wavelength (pixels)\\ (*)non-sinusoidal\\periodic effects}
	& $x$--$\delta x$, $x$ overlap & $1.5$, $2800$ & $0.7$, $3560$ \\\cline{2-4}
	& $x$--$\delta y$, $x$ overlap & $0.4$, $4000$ & $0.0$ \\\cline{2-4}
	& $y$--$\delta x$, $y$ overlap & $0.5$, $4000$ & $0.0$ \\\cline{2-4}
	& $y$--$\delta y$, $y$ overlap & $2.8$, $4000$ & $\sim1$, $\sim13000$(*) \\\hline
	\multirowcell{4}[0pt][l]{Remaining RMS\\(pixels)}
	& $\delta x$, $x$ overlap & $1.4$ & $0.3$ \\\cline{2-4}
