...
CORRECTED_IMG_FILE_LINE_TEXT = 'Corrected Image File : '
HEADER_LINE_END = '--->'
#logger
...
def writeModifiedExposureTimeXMLFile(infile_path,new_ets,new_img_file_path,outfile_path) :
  #make the new header info
  new_header_lines = []
  with open(infile_path,'r') as ifp :
    for line in [l.rstrip() for l in ifp.readlines()] :
      if line.startswith(XML_START_LINE_TEXT) :
        break
      elif GENERATED_BY_LINE_TEXT in line :
        ...
        new_line = line.replace(FILE_LINE_TEXT,CORRECTED_IMG_FILE_LINE_TEXT)
        new_line = new_line.replace(to_replace,os.path.abspath(new_img_file_path))
  ...
  with open(outfile_path,'w') as ofp :
    for line in new_header_lines :
      ofp.write(f'{line}\n')
  with open(outfile_path,'ab') as ofp :
    tree.write(ofp)
  utility_logger.info(f'Wrote out new exposure time xml file {outfile_path}')
