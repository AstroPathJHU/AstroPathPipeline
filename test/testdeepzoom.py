...
class TestDeepZoom(TestBaseSaveOutput):
  ...
  def testDeepZoom(self, SlideID="M206", units="safe", **kwargs):
    ...
    args = [str(root), "--zoomroot", str(zoomroot), "--deepzoomroot", str(deepzoomroot), "--logroot", str(deepzoomroot), "--sampleregex", SlideID, "--debug", "--units", units, "--layers", "1", "--allow-local-edits", "--ignore-dependencies"]
