...
class TestAnnoWarp(TestBaseCopyInput, TestBaseSaveOutput):
  ...
  def testCohort(self, SlideID="M206", units="fast"):
    ...
    args = [os.fspath(root), "--zoomroot", os.fspath(zoomroot), "--logroot", os.fspath(logroot), "--maskroot", os.fspath(maskroot), "--sampleregex", SlideID, "--debug", "--units", units, "--allow-local-edits", "--dbloadroot", os.fspath(logroot), "--ignore-dependencies"]
