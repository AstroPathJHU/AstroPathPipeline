#imports
from astropath.hpfs.image_correction.imagecorrectioncohort import ImageCorrectionCohort
...
class TestImageCorrectionCohort(TestBaseCopyInput, TestBaseSaveOutput) :
    """
    Class to test ImageCorrectionCohort functions
    """

    @classmethod
    def filestocopy(cls) :
        oldroot = folder/'data'
        newroot = folder/'test_for_jenkins'/'imagecorrection'/'root'
    ...
    def outputfilenames(self) :
        ...
        for fns in [fp.name.rstrip(UNIV_CONST.RAW_EXT) for fp in (root2/slide_ID).glob(f'*{UNIV_CONST.RAW_EXT}')] :
            all_fps.append(folder/'test_for_jenkins'/'imagecorrection'/'flatw'/f'{fns}{UNIV_CONST.FLATW_EXT}')
            all_fps.append(folder/'test_for_jenkins'/'imagecorrection'/'flatw'/f'{fns}{UNIV_CONST.FLATW_EXT}01')
        return all_fps
    ...
    def setUp(self) :
        ...
        (folder/'test_for_jenkins'/'imagecorrection'/'root'/slide_ID).mkdir(parents=True,exist_ok=True)
    ...
    def test_image_correction_cohort(self) :
        ...
        for rn in rectangle_ns_with_raw_files :
            args.append(str(rn))
        ImageCorrectionCohort.runfromargumentparser(args=args)
        #compare the results to the reference files
        try :
            for fp in (folder/'data'/'reference'/'imagecorrection').glob('*') :
                if fp.name.endswith('01') :
                    refa = get_raw_as_hw(fp,*(dims[:-1]),np.uint16)
                    testa = get_raw_as_hw(folder/'test_for_jenkins'/'imagecorrection'/'flatw'/slide_ID/fp.name,*(dims[:-1]),np.uint16)
                else :
                    refa = get_raw_as_hwl(fp,*dims,np.uint16)
                    testa = get_raw_as_hwl(folder/'test_for_jenkins'/'imagecorrection'/'flatw'/slide_ID/fp.name,*dims,np.uint16)
            ...
            self.removeoutput()
            shutil.rmtree(folder/'test_for_jenkins'/'imagecorrection')
