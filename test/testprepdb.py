...
class TestPrepDb(unittest.TestCase):
  ...
  def testPrepDb(self, SlideID="M21_1", units="safe", skipannotations=False):
    ...
    args = [os.fspath(thisfolder/"data"), "--sampleregex", SlideID, "--debug", "--units", units, "--xmlfolder", os.fspath(thisfolder/"data"/"raw"/SlideID), "--allow-local-edits", "--ignore-dependencies", "--rerun-finished"]
    if skipannotations:
