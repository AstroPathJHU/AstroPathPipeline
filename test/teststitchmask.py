...
class TestStitchMask(TestBaseCopyInput, TestBaseSaveOutput):
  ...
  def _testStitchMask(self, *, SlideID, masktype, maskfilesuffix, units):
    ...
    args = [os.fspath(root), "--maskroot", os.fspath(maskroot), "--logroot", os.fspath(maskroot), "--mask-file-suffix", maskfilesuffix, "--allow-local-edits", "--sampleregex", SlideID+"$", "--debug", "--ignore-dependencies", "--rerun-finished", "--units", units, "--use-apiddef"]
