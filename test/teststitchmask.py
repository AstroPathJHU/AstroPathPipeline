...
class TestStitchMask(TestBaseCopyInput, TestBaseSaveOutput):
  ...
  def _testStitchMask(self, *, SlideID, masktype, maskfilesuffix):
    ...
    args = [os.fspath(root), "--maskroot", os.fspath(maskroot), "--logroot", os.fspath(maskroot), "--mask-file-suffix", maskfilesuffix, "--allow-local-edits", "--sampleregex", SlideID+"$", "--debug", "--ignore-dependencies"]
