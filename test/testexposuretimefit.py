...
new = readtable(working_dir/f"{args.slideID}_layers_{args.layers[0]}-{args.layers[-1]}_best_fit_offsets_exposuretimefit_test_for_jenkins.csv", LayerOffset, checkorder=True, checknewlines=True)
ref = readtable(folder/"reference"/"exposuretimefit"/f"{args.slideID}_layers_{args.layers[0]}-{args.layers[-1]}_best_fit_offsets_exposuretimefit_test_for_jenkins.csv", LayerOffset, checkorder=True, checknewlines=True)
