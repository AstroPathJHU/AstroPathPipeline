...
def update_source_csv(csv_file):
    ...
    try:
        ...
        dpath = [i.split(',')[1] for i in lines[1:]]
        dname = [i.split(',')[2] for i in lines[1:]]
        spath = [i.split(',')[3] for i in lines[1:]]
...
def ast_gen(mpath, csv_file, mastro_csv):
    ...
    for pos in range(0, len(dname)):
        ...
        patient, batch_id, cohort = extract_specimens(dname[pos], spath[pos], mpath)
        if not patient:
            continue
...
#
def extract_specimens(dname, spath, mpath):
    specimen_path = spath + '\\' + dname + '\\Specimen_Table.xlsx'
    cohort_path = mpath + '\\AstropathCohortsProgress.csv'
...
def next_slide_id(mastro_csv, local_string, st_patient, st_batch_id, proj):
    ...
    if os.path.exists(mastro_csv):
        ...
        slide_id = 1
    #
    # Compare local Astrodef_<project>.csv to Specimen_Table.xlsx and generate
    # SlideIDs for the patient#s not already present in Astrodef_<project>.csv
    #
    if os.path.exists(local_string):
        f = open(local_string, 'r')
        lines = f.readlines()
        f.close()
        at_patient = [i.split(',')[1] for i in lines[1:]]
...
def slide_id_csv(st_patient, st_batch_id, mastro_csv, uppath, proj, cohort):
    ...
    try:
        ...
        time.sleep(10)
        if attempts == 5:
            descriptor = "Cannot open/parse either: {0} or {1}".format(mastro_csv, local_string)
            log_string = "WARNING: " + descriptor + " after " \
                                     + str(attempts) + " attempts."
            print(log_string)
            return
    ...
    attempts = 0
    try:
        with open(local_string, 'a', newline='') as f:
...
def apid_argparser():
    ...
    parser.add_argument('mpath', type=str, nargs='?',
                        default='C:\\Users\\ssotodi1\\Documents\\Demon_Workspace\\Test Setup With md5 fix\\astropath_processing',
...
def start_gen():
    ...
    if not os.path.exists(csv_file):
        print("No AstropathPaths.csv found in " + mpath)
        return
    #
