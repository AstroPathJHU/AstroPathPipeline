...
def shiftplot2D(alignmentset, *, saveasx=None, saveasy=None, figurekwargs={}, plotstyling=lambda fig, ax, cbar, xory: None, island=None, showplot=None):
  ...
  f = fields[0]
  shape = tuple(reversed(floattoint(float(np.max([(f.xvec - x0vec) / deltaxvec for f in fields], axis=0))) + 1))
  ...
  for f in fields:
    idx = (slice(None),) + tuple(reversed(floattoint(float((f.xvec - x0vec) / deltaxvec))))
