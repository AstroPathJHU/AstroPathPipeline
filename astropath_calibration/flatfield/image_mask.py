...
class ImageMask() :
    ...
    def uncompressed_full_mask(self): #the uncompressed mask with the real number of image layers
        ...
        uncompressed_mask = np.ones((*self._compressed_mask.shape,self._layer_groups[-1][1]))
    ...
    def addCreatedMasks(self,tissue_mask,blur_mask,saturation_masks) :
        ...
        if np.min(blur_mask)<1 :
            ...
            self._compressed_mask[:,:,0][enumerated_blur_mask!=0] = enumerated_blur_mask
        ...
        for lgi,lgsm in enumerate(layer_group_saturation_masks) :
            if np.min(lgsm)<1 :
                ...
                self._compressed_mask[:,:,0][enumerated_sat_mask!=0] = enumerated_sat_mask
