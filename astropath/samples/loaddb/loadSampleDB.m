function C=loadSampleDB(root,samp,varargin)
%%-----------------------------------------------
...
%%-----------------------------------------------
    ...
    %
    C = getConfig(root,samp,'loaddb');
    logMsg(C,'loadSampleDB started',1);
    if (C.err==1)
        return
    end
    %------------------------------
    ...
    C.sqlpath = '\\bki02\C\BKI\sql\WSISample\';
    %
    if (opt==-1)
        return
    end
    ...
    %
    logMsg(C,'loadSampleDB finished',1);    
    %
end
