# 5.8.4. Saving Projects for the inForm® JHU Processing Farm
# 5.8.4.1. Description 
...
1.	For example, in the PD1\ PDL1 axis panel we created a segmentation for immune markers and tumor. The markers were PDL1 (520), CD8 (540), FoxP3 (570), Tumor (620), PD1 (650), and CD163 (690). In this panel we select to export the segmentation maps for CD8 (Immune), CD163 (Macrophage), and Tumor. Note that we skip PDL1 since it is designated as an expression marker.
...
**III. Select to export the ‘Component Images (multi-image TIFF)’ for the lowest opal antibody that is only phenotyped once (in the above example this would have been the CD8 algorithm).**
