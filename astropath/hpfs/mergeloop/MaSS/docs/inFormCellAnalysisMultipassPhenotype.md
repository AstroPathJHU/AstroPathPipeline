# inForm Cell Analysis® Multipass Phenotype
## 1. Description
...
## 2.6. Export
Apply the setting according to the protocol laid out [here](SavingProjectsfortheinFormCellAnalysisJHUProcessingFarm.md#saving-projects-for-the-inform-cell-analysis-jhu-processing-farm "Title"). If using the *AstroPath Pipeline* for processing, save the **project AND algorithm*** to the ```\Clinical_Specimen_XX\tmp_inform_data\Projects_Development``` folder. If you do not save the project you will not be able to adjust the phenotyping.
