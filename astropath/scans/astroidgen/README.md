...
```
Additionally, the code intializes the necessary [processing folders](#554-workflow "Title") for each project directory and saves a project level *AstropathAPIDdef_PP.csv* (```PP``` indicates the ```ProjectID```) in the ```upkeep_and_progress``` folder, containing only the specimens related to that project.
...
1. The  ```ProjectID``` and ```CohortID``` are defined in the *AstropathCohortsProgress.csv* file. 
   - A description of this file is contained [here](../../scans/docs/AstroPathProcessingDirectoryandInitializingProjects.md/#451-astropath_processing-directory)
2. The ```BatchID``` is generated for each staining batch 
   - a longer description of this value can be found in the description [here](../../scans/docs/scanning/BatchIDs.md/#446-batchids)
3. New ```SampleName```s are detected from the *SpecimenTable.xlsx* files contained in each cohort folder
   ...
   - A description of the *SpecimenTable.xlsx* file is contained [here](../../scans/docs/scanning/SpecimenTable.md/#442-specimen_table)
...
5. We use path specifiers to shorten descriptions, further description of these paths can be found in the additional documentation [here](../../scans/docs/Definitions.md/#432-path-definitions) repository:
...
Information on these folders is also located in documentation located [here](../../scans/docs/DirectoryOrganization.md/#461-directory-subfolders) repository.
